...
NLTK_FALLBACK_DATA="${NLTK_FALLBACK_DATA:-$HOME/nltk_data}"
...
DEFAULT_MODEL="${DEFAULT_MODEL:-Qwen/Qwen2.5-7B}"
MODELS=(
...
# 去重并过滤空值；若路径不存在则按 HF repo id 处理（允许直接传 "Qwen/Qwen2.5-7B"）
...
EVAL_OUTPUT_ROOT="${EVAL_OUTPUT_ROOT:-$PWD/eval_results}"
